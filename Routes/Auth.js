...
router.post('/foodData', async (req, res) => {
    try {
        ...
        res.send([global.foodData, global.foodCategory, global.userData, global.orderData])
    } catch (error) {
        console.error(error.message)
        res.send("Server Error")
...
router.post('/additem', [
    ...
    try {
        await Item.create({
            ...
            .catch(err => {
                console.log(err);
                res.json({ error: "Enter Item for Auth" })
            })
    } catch (error) {
        console.error(error.message)
    }
...
//options: {half: req.body.options[0], full: req.body.options[1], regular: req.body.options[3], small: req.body.options[4], medium: req.body.options[5], large: req.body.options[6]},
